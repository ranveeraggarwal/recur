...
| Access-state `ConfirmButton` "sized to content" | `ConfirmButton` itself is hardcoded `width: double.infinity` (by design, for its normal full-width uses) and this issue's file list doesn't include `confirm_button.dart`. The design doc's "a `ConfirmButton` sized to content (not full width)" for access states is approximated by wrapping it in `SizedBox(width: 220)`, which clamps the button's requested infinite width down to a fixed, non-full-width size, rather than truly sizing to the label's text width. |
